...
from ldm.models.firststagemodel import FirstStageModel
...
class DDPM(pl.LightningModule):
    ...
    def __init__(self,
        ...
        self.l_cosine_weight = l_cosine_weight
        self.l_lpips_weight = l_lpips_weight
    ...
    def register_schedule(self, given_betas=None, beta_schedule="linear", timesteps=1000,
        ...
        if self.parameterization == "eps":
            ...
            lvlb_weights = 0.5 * np.sqrt(torch.Tensor(alphas_cumprod)) / (2. * 1 - torch.Tensor(alphas_cumprod)) # TODO
        else:
    ...
    def p_mean_variance(self, x, t, clip_denoised: bool):
        ...
        if self.parameterization == "eps":
            ...
            _, mean, std = self.q_sample(x_start=x, t=t, noise=None, return_mean_std=True)
            x_recon = mean * x - std * model_out
        if clip_denoised:
            x_recon.clamp_(-1., 1.)
...
class LatentDiffusion(DDPM):
    ...
    def decode_first_stage_(self, z, predict_cids=False, force_not_quantize=False, output_size=None):
        if predict_cids:
            if z.dim() == 4:
        ...
        if output_size is None:
            if self.default_output_size is not None:
        ...
        if hasattr(self, "split_input_params"):
            if self.split_input_params["patch_distributed_vq"]:
                ...
                if isinstance(self.first_stage_model, FirstStageModel):
                    return self.first_stage_model.decode(z, output_size=output_size)
                else:
    ...
    def decode_first_stage(self, z, predict_cids=False, force_not_quantize=False, output_size=None, requires_grad=False):
        if requires_grad:
            return self.decode_first_stage_(z, predict_cids, force_not_quantize, output_size)
        else:
            with torch.no_grad():
                return self.decode_first_stage_(z, predict_cids, force_not_quantize, output_size)
    ...
    def forward(self, x, c, y, *args, **kwargs):
        t = torch.randint(0, self.num_timesteps, (x.shape[0],), device=self.device).long()
        if self.model.conditioning_key == 'ada' and c is None:
            pass
    ...
    def p_losses(self, x_start, cond, t, y, noise=None):
        ...
        # TODO
        # if self.model.conditioning_key == 'ada' and c is None:
        #     cond = {k : self.q_sample(x_start=v, t=t, noise=noise) for k, v in cond.items()}
    ...
    @torch.no_grad()
    def sample_log(self, cond, batch_size, ddim, ddim_steps, print_bar=True, **kwargs):
        if ddim:
...
class DiffusionWrapper(pl.LightningModule):
    def __init__(self, diff_model_config, conditioning_key):
        ...
        assert self.conditioning_key in [None, 'concat', 'crossattn', 'hybrid', 'adm', 'hybrid-adm', 'ada']
    ...
    def forward(self, x, t, c_concat: list = None, c_crossattn: list = None, c_adm=None):
        ...
        elif self.conditioning_key == 'ada':
...
class SRDiffusion(LatentDiffusion):
    ...
    def get_cond(self, c):
        ...
        if self.model.conditioning_key == 'hybrid':
            ...
            c = rearrange(c, 'b c h w -> b (h w) c')
            c = {f'c_crossattn': [c]}
        elif self.model.conditioning_key =='ada':
            c = {f'c_concat': [c]}
        else:
            raise NotImplementedError
        return c
...
class AdaDiffusion(LatentDiffusion):
    ...
    def get_cond(self, c):
        ...
        if self.c_encode:
            ...
            xc = F.interpolate(c, size=self.image_size, mode='bicubic')
            c = F.interpolate(c, size=self.inp_size, mode='bicubic') # TODO modify size
            encoder_posterior = self.encode_first_stage(c)
            c = self.get_first_stage_encoding(encoder_posterior).detach()
        else:
            # c = F.interpolate(c, size=self.inp_size, mode='nearest') # TODO modify size
        ...
        if self.model.conditioning_key =='ada':
            c = {f'c_concat': [c]}
        else:
            raise NotImplementedError
        return c
    ...
    def training_step(self, batch, batch_idx):
        ...
        self.is_sr = False
        loss, loss_dict = self.shared_step(batch)
        loss_dict = {'Gen_' + k: v for k, v in loss_dict.items()} 
        total_loss = loss_dict['Gen_train/loss_simple']
        self.log_dict(loss_dict, prog_bar=True,
                      logger=True, on_step=True, on_epoch=True)
        ...
        self.is_sr = True
        loss, loss_dict = self.shared_step(batch)
        loss_dict = {'Sr_' + k: v for k, v in loss_dict.items()} 
        total_loss += loss_dict['Sr_train/loss_simple']
        self.log_dict(loss_dict, prog_bar=True,
                      logger=True, on_step=True, on_epoch=True)

        total_dict = {
            'train/loss_simple' : total_loss,
        }
        self.log_dict(total_dict, prog_bar=False, logger=True, on_step=False, on_epoch=True)
    ...
    def validation_step(self, batch, batch_idx):
        self.is_sr = False
        _, loss_dict_no_ema = self.shared_step(batch)
        loss_dict_no_ema = {'Gen_' + k: v for k, v in loss_dict_no_ema.items()} 
        total_loss = loss_dict_no_ema['Gen_val/loss_simple']
        with self.ema_scope():
            _, loss_dict_ema = self.shared_step(batch)
            loss_dict_ema = {'Gen_' + key + '_ema': loss_dict_ema[key] for key in loss_dict_ema}
            total_loss_ema = loss_dict_ema['Gen_val/loss_simple_ema']
        self.log_dict(loss_dict_no_ema, prog_bar=False, logger=True, on_step=False, on_epoch=True)
        self.log_dict(loss_dict_ema, prog_bar=False, logger=True, on_step=False, on_epoch=True)

        self.is_sr = True
        _, loss_dict_no_ema = self.shared_step(batch)
        loss_dict_no_ema = {'Sr_' + k: v for k, v in loss_dict_no_ema.items()} 
        total_loss += loss_dict_no_ema['Sr_val/loss_simple']
        with self.ema_scope():
            _, loss_dict_ema = self.shared_step(batch)
            loss_dict_ema = {'Sr_' + key + '_ema': loss_dict_ema[key] for key in loss_dict_ema}
            total_loss_ema += loss_dict_ema['Sr_val/loss_simple_ema']
        self.log_dict(loss_dict_no_ema, prog_bar=False, logger=True, on_step=False, on_epoch=True)
        self.log_dict(loss_dict_ema, prog_bar=False, logger=True, on_step=False, on_epoch=True)

        total_dict = {
            'val/loss_simple' : total_loss,
            'val/loss_simple_ema' : total_loss_ema,
        }
        self.log_dict(total_dict, prog_bar=False, logger=True, on_step=False, on_epoch=True)
    ...
    def log_images(self, *args, **kwargs):
        self.is_sr = False
        log_gen = super().log_images(*args, **kwargs)
        log_gen = {'Gen_' + k: v for k, v in log_gen.items()} 
        
        self.is_sr = True
        log_sr = super().log_images(*args, **kwargs)
        log_sr = {'Sr_' + k: v for k, v in log_sr.items()} 
        return dict(log_gen, **log_sr)
